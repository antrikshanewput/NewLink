	...
	const { projectName } = await inquirer.prompt([
		{
			...
			validate: (input: string) => {
				...
				if (!validNameRegex.test(input)) return 'Name can only contain letters, numbers, hyphens, and underscores.';
				return true;
			},
		},
	]);
...
async function promptSelectedModules(projectType: 'Monolithic' | 'Microservices'): Promise<ModuleConfig[]> {
	const promptMsg = projectType === 'Monolithic' ? 'Do you want to enable' : 'Do you want to create a microservice for';
...
async function Monolithic() {
	try {
		...
		// Consolidated .env
		const envVars = selectedModules.reduce<Record<string, string>>(
			(acc, mod) => ({
				...acc,
				...(mod.envVars || {}),
			}),
...
async function Microservices() {
	try {
		...
		for (const module of selectedModules) {
			...
			if (module.name === 'authentication') envVars.DB_HOST = 'database';
